...
from mdao.openmdao_components.himmelblau import HimmelblauFunction

if __name__ == "__main__":
    search_domain_x = np.linspace(-4, 4, 101)
    search_domain_y = np.linspace(-4, 4, 101)
    ...
    model.add_subsystem(
        "data",
        subsys=HimmelblauFunction(number_of_points=len(meshgrid_x)),
        promotes=["*"],
    )
    ...
    output_file_path = (
        pathlib.Path(__file__).parent.parent / "data" / "data_himmelblau.csv"
    )
