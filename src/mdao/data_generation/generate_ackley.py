...
from mdao.openmdao_components.ackley import AckleyFunction

if __name__ == "__main__":
    search_domain_x = np.linspace(-5, 5, 101)
    search_domain_y = np.linspace(-5, 5, 101)

    x_for_partials = search_domain_x[int(len(search_domain_x) / 3)]
    y_for_partials = search_domain_y[int(len(search_domain_y) / 3)]
    ...
    model.add_subsystem(
        "data",
        subsys=AckleyFunction(number_of_points=len(meshgrid_x)),
        promotes=["*"],
    )
